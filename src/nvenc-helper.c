...
static GUID get_preset_guid(uint32_t level) {
    switch (level) {
...
{
    HELPER_LOG("Init: %ux%u codec=%u profile=%u bitrate=%u ql=%u rc=0x%x",
               params->width, params->height, params->codec, params->profile,
               params->bitrate, params->qualityLevel, params->rcMode);
    ...
    GUID codecGuid;
    if (params->codec == 0) codecGuid = NV_ENC_CODEC_H264_GUID;
    else if (params->codec == 1) codecGuid = NV_ENC_CODEC_HEVC_GUID;
    else codecGuid = NV_ENC_CODEC_AV1_GUID;

    GUID profileGuid;
    if (params->codec == 0) {
        /* H.264 */
        profileGuid = NV_ENC_H264_PROFILE_HIGH_GUID;
    } else if (params->codec == 1) {
        /* HEVC */
    ...
    GUID presetGuid = get_preset_guid(params->qualityLevel);

    st = enc->funcs.nvEncGetEncodePresetConfigEx(
        enc->encoder, codecGuid, presetGuid,
    ...
    if (params->codec == 1 && params->is10bit) {
    ...
    } else if (params->codec == 2) {
        if (params->is10bit) {
            encConfig.encodeCodecConfig.av1Config.inputBitDepth = NV_ENC_BIT_DEPTH_10;
            encConfig.encodeCodecConfig.av1Config.outputBitDepth = NV_ENC_BIT_DEPTH_10;
        } else {
            encConfig.encodeCodecConfig.av1Config.inputBitDepth = NV_ENC_BIT_DEPTH_8;
            encConfig.encodeCodecConfig.av1Config.outputBitDepth = NV_ENC_BIT_DEPTH_8;
        }
    ...
    if (params->rcMode != 0) {
        /* Mapping VA_RC_* to NV_ENC_PARAMS_RC_MODE.
         * VA_RC_CQP=0x1, VA_RC_CBR=0x2, VA_RC_VBR=0x4, VA_RC_VBR_CONSTRAINED=0x10 */
        if (params->rcMode & 0x1) {
            encConfig.rcParams.rateControlMode = NV_ENC_PARAMS_RC_CONSTQP;
        } else if (params->rcMode & 0x2) {
            encConfig.rcParams.rateControlMode = NV_ENC_PARAMS_RC_CBR;
        } else if (params->rcMode & (0x4 | 0x10)) {
    ...
    if (params->gopLength > 0) {
        ...
        encConfig.gopLength = 0xffffffff;
    }
    encConfig.frameIntervalP = params->ipPeriod > 0 ? params->ipPeriod : 1;
    ...
    initParams.maxEncodeWidth = params->maxWidth > 0 ? params->maxWidth : params->width;
    initParams.maxEncodeHeight = params->maxHeight > 0 ? params->maxHeight : params->height;
    initParams.tuningInfo = NV_ENC_TUNING_INFO_LOW_LATENCY;

    enc->codecGuid = codecGuid;
    ...
    HELPER_LOG("Encoder initialized: %ux%u %s %s (gpu=%s)",
               ...
               params->codec == 0 ? "H.264" : (params->codec == 1 ? "HEVC" : "AV1"),
...
static bool encoder_encode(HelperEncoder *enc, const void *frame_data,
                           ...
                           uint32_t temporal_id, uint32_t pic_type,
...
do_encode:;
    ...
    picParams.pictureType = pic_type;
    if (enc->frameCount == 0 || force_idr) {
    ...
    if (memcmp(&enc->codecGuid, &NV_ENC_CODEC_H264_GUID, sizeof(GUID)) == 0) {
        picParams.codecPicParams.h264PicParams.displayPOCSyntax = (uint32_t)(enc->frameCount * 2);
        picParams.codecPicParams.h264PicParams.refPicFlag = 1;
    } else if (memcmp(&enc->codecGuid, &NV_ENC_CODEC_HEVC_GUID, sizeof(GUID)) == 0) {
        picParams.codecPicParams.hevcPicParams.displayPOCSyntax = (uint32_t)(enc->frameCount * 2);
        picParams.codecPicParams.hevcPicParams.refPicFlag = 1;
        picParams.codecPicParams.hevcPicParams.temporalId = temporal_id;
    } else if (memcmp(&enc->codecGuid, &NV_ENC_CODEC_AV1_GUID, sizeof(GUID)) == 0) {
        picParams.codecPicParams.av1PicParams.displayPOCSyntax = (uint32_t)(enc->frameCount * 2);
        picParams.codecPicParams.av1PicParams.refPicFlag = 1;
        picParams.codecPicParams.av1PicParams.temporalId = temporal_id;
    }
...
static void handle_client(int client_fd)
{
    ...
    while (running) {
        ...
        case NVENC_IPC_CMD_ENCODE: {
            ...
            void *bitstream = NULL;
            uint32_t bsSize = 0;
            bool ok = encoder_encode(&enc, frame, ep.width, ep.height, ep.frame_size, ep.force_idr, ep.temporal_id, ep.pic_type, &bitstream, &bsSize);
            free(frame);
        ...
        case NVENC_IPC_CMD_ENCODE_DMABUF: {
            ...
            picParams.outputBitstream = enc.outputBuffer;
            picParams.pictureStruct = NV_ENC_PIC_STRUCT_FRAME;
            picParams.pictureType = dp.pic_type;
            if (enc.frameCount == 0 || dp.force_idr) {
                picParams.pictureType = NV_ENC_PIC_TYPE_IDR;
            } else if (picParams.pictureType == NV_ENC_PIC_TYPE_UNKNOWN) {
                picParams.pictureType = NV_ENC_PIC_TYPE_P;
            }
            bool needIDR = (enc.frameCount == 0) || dp.force_idr || (enc.frameCount % NVENC_HELPER_IDR_INTERVAL == 0);
            picParams.encodePicFlags = needIDR
                ? (NV_ENC_PIC_FLAG_OUTPUT_SPSPPS | NV_ENC_PIC_FLAG_FORCEIDR)
                : 0;
            ...
            if (memcmp(&enc.codecGuid, &NV_ENC_CODEC_H264_GUID, sizeof(GUID)) == 0) {
                ...
                picParams.codecPicParams.hevcPicParams.displayPOCSyntax = (uint32_t)(enc.frameCount * 2);
                picParams.codecPicParams.hevcPicParams.refPicFlag = 1;
                picParams.codecPicParams.hevcPicParams.temporalId = dp.temporal_id;
            } else if (memcmp(&enc.codecGuid, &NV_ENC_CODEC_AV1_GUID, sizeof(GUID)) == 0) {
                picParams.codecPicParams.av1PicParams.displayPOCSyntax = (uint32_t)(enc.frameCount * 2);
                picParams.codecPicParams.av1PicParams.refPicFlag = 1;
                picParams.codecPicParams.av1PicParams.temporalId = dp.temporal_id;
            }
            picParams.frameIdx = (uint32_t)enc.frameCount;
            picParams.inputTimeStamp = enc.frameCount;
...
dmabuf_cleanup:
        ...
        case NVENC_IPC_CMD_ENCODE_SHM: {
            ...
            bool ok = encoder_encode(&enc, shm_ptr, sp.width, sp.height,
                                     sp.frame_size, sp.force_idr, sp.temporal_id, sp.pic_type,
                                     &bitstream, &bsSize);
